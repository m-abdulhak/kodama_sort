...
class ThreePi:
    ...
    # The following works, but there is no LCD on the kodama.
    #def print_to_lcd(self, str):
    #    """This requires the LCD screen to be installed."""
    #    self.gateway.write(CLEAR_LCD_CMD)
    #    self.gateway.write(PRINT_CMD)
    #    n = len(str)
    #    self.gateway.write(chr(n))
    #    self.gateway.write(bytes(str))
